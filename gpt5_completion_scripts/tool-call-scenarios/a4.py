...
# Pre-load ROUGE scorer globally for performance
ROUGE_SCORER = evaluate.load('rouge')

# Global system prompt cache
SYSTEM_PROMPT_CACHE = None

# Global storage for latest generated quiz (to handle max turns exceeded)
LATEST_GENERATED_QUIZ = None
...
# Load original quizzes once at startup - 100 rows for testing
ORIGINAL_QUIZZES = load_original_quizzes(sample_size=100, random_seed=42)

def get_system_prompt():
    """Get cached system prompt or load it once"""
...
def evaluate_quiz_quality(generated_quiz: str, quiz_index: int) -> str:
    ...
    global LATEST_GENERATED_QUIZ, CURRENT_QUIZ_INDEX
    
    # Store the latest generated quiz for this index (in case of max turns exceeded)
    if quiz_index == CURRENT_QUIZ_INDEX:
        LATEST_GENERATED_QUIZ = generated_quiz
    ...
    # Use global ROUGE scorer for better performance
    ROUGE_SCORER.add_batch(predictions=predictions, references=references)
...
async def generate_quiz_with_refinement(quiz_index: int):
    ...
    global CURRENT_QUIZ_INDEX, LATEST_GENERATED_QUIZ
    
    if quiz_index >= len(ORIGINAL_QUIZZES):
        raise ValueError(f"Quiz index {quiz_index} out of range")
    
    # Set current quiz index and reset latest quiz storage
    CURRENT_QUIZ_INDEX = quiz_index
    LATEST_GENERATED_QUIZ = None
...
Follow the format and style guidelines in your instructions."""
    ...
    try:
        ...
        # If max turns exceeded, use the latest generated quiz we captured locally
        if "Max turns" in error_msg and "exceeded" in error_msg:
            print(f"  Quiz {quiz_index + 1}: Reached max turns, using latest generation...")
            if LATEST_GENERATED_QUIZ:
                print(f"  ✓ Successfully retrieved latest quiz from local storage")
                return LATEST_GENERATED_QUIZ
            else:
                print(f"  ✗ No quiz found in local storage")
        
        print(f"  Exception for quiz {quiz_index + 1}: {error_msg[:100]}...")
        
        # Simple fallback - generate without evaluation tools (for other errors)
        try:
...
async def batch_generate_quizzes(num_quizzes=None):
    """Generate multiple quizzes sequentially to avoid rate limits"""
    if num_quizzes is None:
        num_quizzes = len(ORIGINAL_QUIZZES)
    
    results = []
    print(f"Processing {num_quizzes} quizzes sequentially...")
    
    for i in range(min(num_quizzes, len(ORIGINAL_QUIZZES))):
        try:
            print(f"Starting quiz {i+1}/{num_quizzes}...")
